...
"""
TODO: (Sailbot)
This file is just shamelessly copied from the GitHub repo
below. Figure out how to use this library in a cleaner way
"""
...
AISchars = r'@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_ !"#$%&\'()*+,-./0123456789:;<=>?'
re_AISchars = {}
...
class AISStaticAndVoyageReportMessage(AISMessage):
    def __init__(self, id=5, repeat=0, mmsi=0, ais_version=0, imo=0, callsign=0, shipname=0,
        ...
        super(AISStaticAndVoyageReportMessage, self).__init__({
            ...
            'imo': ["uint", 30, imo],
            'callsign': ["uint", 42, AISString2Bits(callsign, length=42 / 6).int
                         if isinstance(callsign, str) else callsign],
            'shipname': ["uint", 120, AISString2Bits(shipname, length=120 / 6).int
                         if isinstance(shipname, str) else shipname],
            ...
            'destination': ["uint", 120, AISString2Bits(destination, length=120 / 6).int
...
class AISStaticDataReportBMessage(AISMessage):
    def __init__(self, id=24, repeat=0, mmsi=0, partno=1, shiptype=0,
        ...
        super(AISStaticDataReportBMessage, self).__init__({
            ...
            'partno': ["uint", 2, partno],
            'shiptype': ["uint", 8, shiptype],
            'vendorid': ["uint", 18, AISString2Bits(vendorid, length=18 / 6).int
                         if isinstance(vendorid, str) else vendorid],
            'model': ["uint", 4, model],
