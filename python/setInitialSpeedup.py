...
if __name__ == "__main__":
    ...
    while numConnections < MIN_NUM_SPEEDUP_SUBSCRIBERS_BEFORE_STARTING:
        rospy.loginfo("{} speedup subscribers found. Waiting for at least {} speedup subscribers before publishing initial speedup".format(numConnections, MIN_NUM_SPEEDUP_SUBSCRIBERS_BEFORE_STARTING))
        time.sleep(0.1)

        # Calculate number of connections at the end of each loop
        numConnections = speedupPublisher.get_num_connections()

    rospy.loginfo("{} speedup subscribers found, which is greater than or equal to the minimum required {}. Publishing initial speedup of {} now".format(numConnections, MIN_NUM_SPEEDUP_SUBSCRIBERS_BEFORE_STARTING, initial_speedup))
